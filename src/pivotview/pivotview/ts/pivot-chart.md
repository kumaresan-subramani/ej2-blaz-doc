...
End user can bind both local and remote data binding options available in the component to feed the data. The [`dataSource`](https://ej2.syncfusion.com/documentation/api/pivotview/dataSourceSettingsModel/#datasource) property can be assigned either with an instance of `DataManager` or  JavaScript object array collection.
...
{% tab template="pivot-table/pivot-table", s5Template="multi-axes-scrollbar", sourceFiles="index.ts,index.html" %}
...
let pivotTableObj: PivotView = new PivotView({
    ...
    displayOption: { view: 'Chart' },
    chartSettings: { enableMultiAxis: true,
